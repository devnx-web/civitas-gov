...
export interface MaterialListItem {
  ...
  ativo: boolean;
  unidadeMedida: { id: string; codigo: string; nome: string } | null;
  grupo: null;
  subclasse: null;
  criadoEm: Date;
  atualizadoEm: Date;
...
export async function listarMateriais(
  ...
  const rows = await prisma.material.findMany({
    where,
    include: { unidadeMedida: { select: { id: true, codigo: true, nome: true } } },
...
export async function criarMaterial(data: any) {
  const existe = await prisma.material.findFirst({
    where: { tenantId: data.tenantId, codigo: data.codigo },
  });
  if (existe) throw new Error("Já existe um material com este código.");
  return prisma.material.create({ data });
}
export async function atualizarMaterial(tenantId: string, id: string, data: any) {
  return prisma.material.update({ where: { id }, data });
}
export async function excluirMaterial(tenantId: string, id: string) {
  return prisma.material.update({ where: { id }, data: { ativo: false } });
}
