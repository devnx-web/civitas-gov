...
export default function LGPDPage() {
  ...
  async function handleAnonimizar(id: string) {
    if (!confirm("Anonimizar todos os dados deste titular? Esta ação não pode ser desfeita."))
      return;
              ...
              <table className="w-full text-sm">
                <thead className="bg-muted">
                  <tr>
                    ...
                    <th className="px-3 py-2 text-left">Consentimento</th>
                  ...
                  {titulares.map((t) => (
                    <tr key={t.id} className="border-b">
                      ...
                      <td className="px-3 py-2">
                        {t.consentimentos[0]?.concedido ? (
                          <span className="text-green-600 text-xs">✅ Ativo</span>
                        ) : (
                          <span className="text-red-600 text-xs">❌ Revogado</span>
                        )}
                      </td>
                      <td className="px-3 py-2 text-right space-x-2">
                        <button
                          onClick={() => handleConsentimento(t.id)}
                          className="text-xs text-primary hover:underline"
                        >
                          Consentimento
                        </button>
                        <button
                          onClick={() => handleAnonimizar(t.id)}
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted">
              <tr>
                ...
                <th className="px-3 py-2 text-left">Dados Afetados</th>
