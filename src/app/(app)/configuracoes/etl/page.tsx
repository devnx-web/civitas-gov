    ...
    <main id="main-content" className="space-y-6 p-6">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-500">
          Importe dados via CSV para o banco de dados. Os registros são inseridos ou
          atualizados (upsert) conforme a chave única de cada entidade.
        </p>
      </div>
      ...
      {preview && preview.headers.length > 0 && (
        <section className="rounded-xl border border-ink-200 bg-white p-5 dark:border-ink-800 dark:bg-ink-950">
          ...
          <p className="mt-2 text-xs text-ink-400">
            Mostrando até 5 linhas de visualização.
          </p>
        </section>
      )}
      ...
      {csvContent && (
        <div className="flex gap-3">
          ...
          <Button variant="secondary" onClick={handleReset} disabled={isPending} aria-disabled={isPending}>
            Cancelar
          </Button>
        </div>
      )}
