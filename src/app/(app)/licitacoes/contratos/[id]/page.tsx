...
export async function generateMetadata({
  ...
  return { title: c?.objeto ?? "Detalhes do contrato" };
}
...
export default async function ContratoDetalhesPage({
  ...
  const [contrato, documentosAssinaveis] = await Promise.all([
    obterContrato(id, tenantId),
    obterDocumentosPorEntidade(tenantId, "contrato", id),
  ]);
